...
- El usuario elige el motor: **OpenAlex**, **Semantic Scholar**, **Crossref**, **Europe PMC**, **ERIC (educación)** o **Todos** (busca en todos en paralelo y combina sin duplicados por DOI). Crossref, Europe PMC y ERIC no requieren registro ni token.
- ERIC no entrega DOI ni citas → sus resultados muestran "Ver ficha ↗" (enlace a eric.ed.gov) en lugar de la vista de detalle enriquecida.
- arXiv: descartado por decisión de la autora (son preprints). DBLP (computación): pendiente — su API estaba caída (500/503) al momento de integrar; el registro de motores permite sumarlo con un servicio + mapper + 1 línea.
